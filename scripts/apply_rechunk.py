"""应用知识库文档重新切片结果，并重建该文档 Chroma 索引。
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="应用知识库文档重新切片结果，并重建该文档 Chroma 索引。",
    )
    parser.add_argument("--doc-id", required=True, help="要应用重新切片的文档 doc_id。")
    parser.add_argument(
        "--chunk-size",
        type=int,
    ...
    parser.add_argument(
        "--yes",
        action="store_true",
        help="确认写回 SQLite/FTS5 并重建 Chroma；不传则拒绝执行。",
    )
    args = parser.parse_args()
    if not args.yes:
        parser.error("apply rechunk modifies SQLite/FTS5 and Chroma; pass --yes to confirm")
    return args
...
def print_text_report(report: dict) -> None:
    ...
    print(f"reindexed_to_chroma: {report['reindexed_to_chroma']}")
